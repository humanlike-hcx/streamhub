...
## 2026-06-11 Redis 弹幕发送限流

### 本次目标

给 Netty WebSocket 弹幕发送加 Redis 限流，避免单个用户在同一个视频房间高频刷屏，造成广播放大。
...
```text
streamhub:danmaku:rate:{videoId}:{userId}
```
...
streamhub:
  danmaku:
    ...
    rate-limit-max-messages: 3
```

含义：

```text
同一个用户在同一个视频房间内，5 秒最多发送 3 条弹幕。
```

### Redis 原子操作

使用 Lua 脚本把 `INCR` 和 `EXPIRE` 放在一次 Redis 执行里：
...
- 如果应用服务挂了：
  - WebSocket 连接会断开。
  - 用户需要重连。
  - Redis 中的限流 key 会按 TTL 自动过期。

- 如果 Redis 挂了：
  - 当前实现会抛出异常并关闭连接。
  - 后续可以优化为降级策略：
    - 保守策略：Redis 不可用时拒绝发送弹幕。
    - 可用性优先策略：Redis 不可用时放行，但记录 warn。

当前更适合采用保守策略，因为弹幕广播有放大效应，Redis 挂了还无脑放行可能引发广播风暴。

### 和令牌桶的区别

- 固定窗口：
  - 实现简单。
  - 适合“5 秒最多 3 条”这种用户级发送限制。
  - 边界处可能出现短时间突刺。

- 滑动窗口：
  - 更精确。
  - 可以避免固定窗口边界突刺。
  - Redis 实现通常用 ZSet，成本更高。

- 令牌桶：
  - 令牌按固定速率生成。
  - 桶里有令牌就允许请求。
  - 允许一定突发流量。
  - 更适合 API 网关、接口 QPS、上传请求这类平滑限流。
